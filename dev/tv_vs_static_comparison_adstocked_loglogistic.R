...
cache_dir <- "data-raw/tv_vs_static_cache_adstocked_loglogistic"
...
say("Fitting static fit_response(type='log_logistic') on adstocked spend...")
fit_static <- fit_response(
  weekly, spend = "spend_adstocked", kpi = "kpi", date = "date", type = "log_logistic",
  chains = 4, iter = 4000, warmup = 1000, refresh = 0,
  file = file.path(cache_dir, "static_log_logistic")
)

say("Fitting fit_response_tv(type='log_logistic', varying='e') on adstocked spend...")
fit_tv <- fit_response_tv(
  weekly, spend = "spend_adstocked", kpi = "kpi", date = "date", type = "log_logistic",
  varying = "e", method = "spline",
  chains = 4, iter = 4000, warmup = 1000, refresh = 0,
  file = file.path(cache_dir, "tv_log_logistic")
)
...
p_monthly <- ggplot(snap_curves, aes(x = spend, y = center, group = label, color = date)) +
  ...
  labs(title = "Linear TV (adstocked spend, log_logistic): static curve vs. every monthly TV snapshot",
...
ggsave(file.path(plot_dir, "01_static_vs_all_monthly_snapshots_adstocked_loglogistic.png"), p_monthly, width = 10, height = 6.5)
